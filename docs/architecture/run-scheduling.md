...
4. `maxConcurrency - max(实时 claim 推导的 busySlots, 平台活动 RunAttempt 数) > 0`；没有实时
   claim 提示时回退到最近心跳的 `busySlots`；
5. CPU 使用率不超过阈值；
6. 内存使用率不超过阈值；
...
调度器依次处理稳定排序的等待用例。首次 attempt 每分配一个用例就立即减少该 Runner 的模拟空闲槽位并重新评分，因此会自然向更空闲的节点扩散；分数相同时按 Runner ID 排序。重试 attempt 则从上一次 Runner 的下一个稳定 Runner ID 开始严格轮询，仍先应用在线、兼容、指标和容量准入；下一个节点不可用时继续向后查找，全部候选耗尽后循环，只有一台合格节点时才复用。Adapter 环境地址以首轮地址为起点，按 attempt 编号在批次固化的完整环境池中执行相同循环。公开日志页的单用例诊断执行虽然会创建新的隐藏批次和 runId，但调度前会通过父批次与原始 ExecutionRun 关联读取该用例族最近一次 Runner，并从 assignment 的固化执行规格读取实际 Adapter 地址，因此跨诊断批次仍从两者的下一项继续轮询。

任务并发是“允许的最大在途数”，不是必须维持的目标值。算法没有固定预留槽位：已认证 claim 携带
Agent 当前计算的空闲槽位，用于纠正最多落后一个周期的心跳容量；平台活动 reservation 始终作为
占用下限，不能被该提示覆盖。健康且未越过资源阈值时可一直填充到任务、项目和 Runner 三者中最小
的并发上限。Agent 本地 attempt 结束会唤醒领取循环，空响应的服务端退避不再形成固定补槽空窗。
运行中短时低于上限仍可能来自 CPU、内存、单位 CPU 负载、指标新鲜度准入、恢复屏障或可运行队列
耗尽；可在总体/单 Runner 调度日志的稳定阻塞原因中区分，不得为追求表面满载绕过资源保护。
...
- claim 实时容量（缺失时使用心跳 `busySlots`）与平台活动 attempt 可能描述同一工作，容量计算取
  二者最大值而不是相加，避免重复扣减；仓储在 reservation 事务中重复应用该规则以防并发超卖。
- Full 在同一 PostgreSQL 事务中保存调度事实和 outbox，独立 Dispatcher 将消息幂等发布到 JetStream；SQLite 与 JetStream 运行相同的至少一次投递契约，Redis 不参与正确性判断。

## 失败重跑边界

`retryLimit` 表示首次执行之外允许的用例失败重跑次数。例如配置 2 时，TestNG 失败的 attempt 1 和 attempt 2 后重新排队，attempt 3 失败后形成该用例最终结果。普通失败与 Runner/传输异常的后续 attempt 都按 Runner 和 Adapter 环境池轮询；Runner/传输异常另有两次恢复预算，并持续避开已产生基础设施异常的节点，不消耗这里的重跑次数。Agent 完成上报在权威事务中固化终态、状态事件和下一次 attempt；重复或迟到上报不会覆盖新租约持有者的结果。

整轮模式可保存有序动态并发阶段。上下文使用实际执行轮次（首轮为 1、第一次重跑为
2）、上一轮仅终态 attempt 的通过率，以及当前轮仍处于 queued/assigned/running 的用例数。首个
命中阶段写入批次级权威状态并持续作为在途上限；每条规则只在自己的 `executionRound` 内判断，
该轮结束前未命中时不在后续轮次补判。只有列表中位于当前阶段之后的规则在其指定轮次命中才会切换。全部规则
从未命中时才使用任务基础并发。规则随 `RunBatch.policy` 固化，活动阶段在 Lite/Full 数据库中持久化，
因此任务修改、Web/worker 重启或 Full 副本切换都不会改变既有批次的并发状态。
...
这些时间同样位于批次计划开始与最终更新时间之间，因此计入任务总执行时长。详情查询不读取或
返回恢复凭据密文。

配置测试复用同一 URL、Basic 鉴权、重定向拒绝和 10 秒超时边界，但只读取 job `api/json` 的名称、
可构建状态、排队状态和 `lastBuild` 摘要，不访问 Rebuilder endpoint。未保存密钥直接用于该次请求；
留空时只允许服务端解密该任务同规则 ID 的已保存密钥，响应和审计均不包含密钥。
...
Jenkins Pipeline 使用服务账号签发的最小权限 `af_api_` API Key。`POST /api/v1/jenkins/runs` 需要项目级 `run.create`，只接受 `suiteId`，返回批次 ID、30 秒建议轮询周期、带批次绑定 HMAC 的七天只读进度轮询 API 地址，以及不带过期时间的永久只读详情地址（`progressUrl` 与 `resultUrl` 相同）。Jenkins 步骤必须轮询到批次终态才结束，并打印当前轮次、本轮完成/通过/失败、累计通过、最终失败和详情链接；启动、超时及终态均链接到同一详情页，返回值 `progressUrl` 与 `resultUrl` 也保持一致。正常用例失败不使 Jenkins 步骤失败，执行异常或中断才失败。

`GET /api/v1/run-batches/{batchId}/progress` 可使用 `run.read` API Key，或使用只绑定该批次的临时/永久签名参数。旧版 `/progress/{batchId}` 继续支持七天临时令牌并只渲染进度卡片；`/share/run/{token}` 使用永久令牌，复用执行历史详情的批次概览、轮次汇总、结果图表和用例表格。执行记录对排队、运行、完成、异常和取消批次统一提供生成按钮。公开页不渲染应用顶栏、侧栏、执行控制或需要额外鉴权的日志、产物、导出与调度事件入口。签名参数不能访问其他批次或普通鉴权 API。永久链接依赖部署主密钥保持稳定，删除对应批次后失效。

## 调度事件完整性

`scheduling_events` 是只追加的诊断流水（批次调度汇总、run 分配、领取/完成、资源快照与轮次恢复）。自 PostgreSQL 0050 / SQLite 0051 迁移起，该表不再以外键引用 `run_batches`、`runners`、`execution_runs` 与 `run_attempts`：

- 事件总在引用对象创建或校验之后由同一应用流程写入，引用完整性由写入路径保证。
- 外键检查会让每条事件写入对父表行持有共享锁，与完成上报事务的批次行排他锁互相排队，高并发下显著拖慢调度、领取与完成链路；移除后事件写入不再阻塞执行热路径。
- 批次被保留周期删除时，诊断流水由保留周期显式清理（PostgreSQL 与 SQLite 行为一致）；执行机注销采用墓碑式 purge，历史记录中的事件引用保留。
