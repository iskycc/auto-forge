...
任务并发是“允许的最大在途数”，不是必须维持的目标值。算法没有固定预留槽位：候选 Runner 的
心跳与平台活动 attempt 口径取最大值，健康且未越过资源阈值时可一直填充到任务、项目和 Runner
三者中最小的并发上限。运行中短时低于上限通常来自用例完成与下一次 claim/心跳之间的可见窗口，
或 CPU、内存、单位 CPU 负载、指标新鲜度准入；可在总体/单 Runner 调度日志的稳定阻塞原因中区分，
不得为追求表面满载绕过资源保护。
