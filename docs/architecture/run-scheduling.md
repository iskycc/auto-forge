...
`GET /api/v1/run-batches/{batchId}/progress` 可使用 `run.read` API Key，或使用只绑定该批次的临时/永久签名参数。`/progress/{batchId}` 使用七天临时令牌并只渲染进度卡片；`/share/run/{token}` 使用永久令牌，复用执行历史详情的批次概览、轮次汇总、结果图表和用例表格。执行记录对排队、运行、完成、异常和取消批次统一提供生成按钮。公开页不渲染应用顶栏、侧栏、执行控制或需要额外鉴权的日志、产物、导出与调度事件入口。签名参数不能访问其他批次或普通鉴权 API。永久链接依赖部署主密钥保持稳定，删除对应批次后失效。
