...
调度器依次处理稳定排序的等待用例。首次 attempt 每分配一个用例就立即减少该 Runner 的模拟空闲槽位并重新评分，因此会自然向更空闲的节点扩散；分数相同时按 Runner ID 排序。重试 attempt 则从上一次 Runner 的下一个稳定 Runner ID 开始严格轮询，仍先应用在线、兼容、指标和容量准入；下一个节点不可用时继续向后查找，全部候选耗尽后循环，只有一台合格节点时才复用。Adapter 环境地址以首轮地址为起点，按 attempt 编号在批次固化的完整环境池中执行相同循环。
