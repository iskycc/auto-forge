...
消失且没有活动批次快照后才回收上传对象。完整环境池在批次创建时写入不可变快照；首轮按任务中
稳定的用例顺序分配起点，后续 attempt 从该起点沿环境池轮询。上传不设置固定业务大小上限，但始终受对象存储、磁盘
和执行协议配额保护。

用例任务的 `policy.projectVersionId` 是批量执行的权威版本关联。新任务只接受同项目的活跃版本，
成员写入按项目和版本同时校验；已有成员时，只有全部成员都属于目标版本才允许切换。批次把该字段
固化进执行策略快照，列表查询在分页前按其过滤。页面顶栏只决定当前可见范围，不向执行请求增加
第二份版本覆盖；计划、Jenkins 和普通 API 仍只提交 `suiteId`，服务端预检统一拒绝缺失、归档或
成员不一致的版本关联。
