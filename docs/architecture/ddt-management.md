...
可选 `cursor` 和 1–20 的 `limit`；可选 `indexOffset`（0–19）启用分页统计，表示之前已匹配数量除以 20 的余数。原有 CaseID/字段条件筛选不变。新标签不预加载用例列表、统计、导入或回收站，
...
缓存沿用会话内存的容量、过期和身份范围限制，以范围、关键词及搜索批次隔离统计与已读取页面；新搜索生成新批次避免复用旧页。
关键词和页码写入 URL，浏览器前进后退、重返标签复用缓存；主动提交重新统计当前数据，切换标签或范围取消在途检索。
