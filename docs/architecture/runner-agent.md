...
URL 型运行时输入不再下发外部下载地址。主平台从已授权的不可变 assignment 快照解析 URL，在节点本地流式缓存、校验后，通过同一输入端点分发；上传对象也改为流式读取，避免整包进入 Web 内存。Runner 对 `jdk-archive` 和 `jar-bundle` 使用独立于批次目录的持久缓存，按 SHA-256 和大小寻址，使用后从该时刻续期 24 小时；详见[缓存生命周期与升级说明](./runtime-input-cache.md)。
...
资源监控读取心跳中的 CPU、内存、负载和槽位信息。SQLite `0071_runner_resource_samples.sql` 与
PostgreSQL `0069_runner_resource_samples.sql` 在原心跳 UPDATE 的短事务内采样，按平台接收时间每分钟
保留第一份资源样本；每节点循环覆盖 360 个固定槽位。没有独立维护线程、额外写事务或 Redis 事实依赖，
跨主平台写入通过数据库约束及条件更新去重。查询最多读取 360 行，仅返回最近 6 小时内样本；延迟到达的
旧采样不能覆盖新槽位，purge 删除对应监控记录。趋势不用于调度判定，调度仍读取最新资源快照。

两种迁移都只新增表、函数/触发器，不回填旧数据；全新安装、上版本升级和事务失败回滚有契约测试。
迁移失败由迁移事务整体回滚，排除错误后重启重试。回退应用版本可保留新表和触发器，旧版本心跳字段
仍兼容；需要删除历史设施时必须在停机备份后同步处理触发器、表和迁移记录，不修改已发布迁移摘要。

显式授权的 PTY 会话采用 `argv[0] = -<shell>` 与 `-i`，让配置的 Shell 执行自身的登录初始化规则；
Bourne 兼容 Shell 读取 `/etc/profile`，其他 Shell 以各自文档为准。白名单过滤仍在进程启动前执行，
平台/Agent 凭据不从服务环境继承；用例执行路径不受此改动影响。系统 profile 自身的错误或阻塞会影响
Shell 初始化，可通过宿主机运维修复，不静默跳过 profile。
