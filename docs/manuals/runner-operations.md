...
项目设置可分别流式上传 JDK 压缩包与包含用例全部依赖的 JAR 压缩包，也可登记主平台可访问的
HTTP(S) 链接、精确字节数和 SHA-256；上传没有固定业务大小上限，但对象存储与执行工作区配额仍
必须生效。Adapter 的启用状态、Suite/Test 和多个环境地址在创建或编辑用例任务时填写；首轮按
用例顺序分散环境地址，同一用例的后续 attempt 按完整环境池轮询并固化到执行规格。Runner 下载后再次校验摘要，在 attempt 工作目录内拒绝路径穿越、
