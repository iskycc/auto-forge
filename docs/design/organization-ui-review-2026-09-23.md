...
- v1.17.23 的 Release checks 与发布包各离线业务场景已通过。独立 Full 500 执行槽压力任务首次因并发 JAR 导入的 PostgreSQL 锁等待超时失败，保持代码和断言不变复验通过；保留首次失败记录，这不表示锁等待问题已被本次 UI 变更消除。
- 源码 CI 的真实 Agent 场景两次停在选择 JAR 后扫描按钮仍禁用的位置，未进入 Agent 执行。该用例改为复用现有 `selectJarForInspection`，在 15 秒范围内确认文件已被页面接收，再进行扫描；不重试扫描、执行或业务断言。此补充仅修改测试与报告，不改变 v1.17.23 已发布镜像的产品代码，真实 Agent 流程重新交由 CI 验证。
