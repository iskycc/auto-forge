...
发布后的后续检查见 [v1.17.19 UI 细节与操作体验复查](ui-detail-audit-v1.17.19-2026-09-22.md)，范围扩展到业务工作台，并单独区分当前问题与已修复项。
...
发布前补充执行完整 TypeScript 单元测试：180 个文件、1012 项通过；根目录格式、lint、类型与 E2E 覆盖清单检查通过。首轮 CI 仅全站布局遍历因固定保存栏与滚动表单的矩形相交而失败，本次新增的六项平台操作场景及全部 Full 分布式验收均通过。共享布局检查器随后增加固定/粘性层的实际命中判断，只豁免浮动层覆盖普通文档；新增反例继续拒绝普通控件、同一浮动层内和不同浮动层间的真实重叠。反例与全站 23 条页面路由的双视口遍历本地通过，未修改产品布局或添加失败自动重试。新截图保存在 `.local/release-1.17.19/layout-screenshots/`，配置页 1024px、1536px 已实际查看。

第二轮 CI 的布局场景已通过，断网 Lite 资产场景暴露另一处验收等待缺口：CSV 已读取成功，但全范围批选使用的后台快照尚未同步，页面明确提示“当前范围正在后台准备，请稍后重试”。用例导入验收改为仅对该提示在三十秒内重新点击只读的“解析并预览”，其他错误立即失败；不重放导入、勾选或加入任务写操作。浏览器注入未同步快照后验证提示与恢复，随后完整 JAR 导入、执行闭环本地通过。产品快照及资源优先级行为保持不变。

第三轮完整 CI 通过；Full 后台负载验收发现质量洞察顶部方法数为零、趋势图却已有执行记录。追踪确认快照已生成，不能靠延长浏览器等待修复：Full 事实补齐使用 `SKIP LOCKED` 跳过并发构建中的记录，且汇总各项指标使用不同数据库读快照。新增真实双连接回归，修复前五项全部稳定失败；Lite/Full 统计读取改为一致的只读事务，Full 通过事实主键幂等补齐，不再锁住或跳过完成记录。修复后相关三文件三十项集成测试通过。回归包含并发提交期间总数、趋势、耗时保持一致、下一次读取看到新结果，以及非键行锁不再导致统计漏计；已纳入 Full 适配器和分布式契约验收。
