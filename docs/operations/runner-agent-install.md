...
1. 在“平台配置”设置 Runner 能够访问的“内部访问地址（Runner）”；可信内网可直接填写 `http://内网IP:端口`，跨不可信网络应使用 HTTPS。私有 PKI 场景准备 PEM CA 链。内部地址保存后立即用于后续安装或重新安装，无需重启；旧配置未填写内部地址时兼容回退到外部访问地址。
...
更换工作目录只影响安装或明确配置后的重新安装：原目录中的本地身份、凭据与 spool 不会自动迁移。更换后执行机会作为新节点重新注册，旧身份应先撤销或注销。

最多 50 台的批量升级只上传并替换 `/opt/autoforge/bin/autoforge-agent` 与 `/opt/autoforge/lib/cotest-testng-adapter.jar`，然后重启并健康检查服务。`config.json`、私有 CA、systemd unit、运行账号、身份目录和数据目录均不被重写；升级前仍保存一组完整且一致的 `.autoforge-previous` 文件用于回滚。需要修改配置时，应回到自动安装面板，完成探测、指纹确认和配置填写后执行重新安装，或在单机更新中明确提交本轮需要变更的部署配置。
