...
```bash
chmod 0755 autoforge-agent-0.2.2-amd64
./autoforge-agent-0.2.2-amd64 version
```
...
./autoforge-agent-0.2.2-amd64 doctor
...
SOURCE_DATE_EPOCH=0 AUTOFORGE_RELEASE_REVISION=local \
  bash scripts/release/build-agent.sh 0.2.2 amd64 dist/release
