...
公开日志使用与控制台一致的卡片、状态标签、信息说明和日志正文区域。全平台默认浅色，并通过同一 ConfigProvider 的 Ant Design darkAlgorithm 支持深色；业务布局和日志颜色均从相同语义 token 派生，浏览器偏好通过非敏感 Cookie 保存并参与服务端首屏渲染，不新增数据库、外部请求或运行依赖。执行机交互终端使用自身终端调色板，不影响公开日志主题。
