...
2. 首次启动默认生成可单独运行的 Lite 配置、主密钥和不同用途的随机令牌。首位管理员令牌单独写入 `initial-admin-token`，创建管理员后删除。Web 与 Full worker 读取同一配置文件；进程监听、基础设施和容量参数修改后必须重启组合根。平台时区、外部访问地址与 Runner 内部访问地址按请求读取；外部地址用于匿名分享、导出和 Jenkins 链接，内部地址用于后续 Runner 安装，缺失时兼容回退到外部地址。产物开关在创建新批次时读取并固化进策略快照，因此这些配置无需重启。平台时区缺失时兼容回落到 `Asia/Shanghai`，但不改变 UTC 持久化与协议语义。
