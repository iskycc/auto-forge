...
三个平台及其 worker 自动使用中间件主机上的共享 PostgreSQL 作为业务时间基准，无需配置
节点时间优先级。浏览器耗时通过平台时间计算；“平台设置 → 系统诊断”可检查当前接入节点的
时间源和宿主机偏差。采样短暂中断可保留 120 秒，超期 readiness 不就绪并返回 503。

五台主机和 Runner 仍应配置可靠 NTP，TLS、S3 签名及中间件 TTL 仍依赖系统时间。
首次升级应排空任务并更新 Runner；数据库需要大幅调时前，应排空执行并停止全部平台/worker，
统一校时后整体重启。详情见 [ADR 0013](../../../docs/adr/0013-platform-time-authority.md)。
