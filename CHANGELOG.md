...
- 未登录首页按方案 A“可信控制面”完全重写：浅色首屏突出可控、可追溯的产品定位，使用真实公开聚合
  数据呈现批次状态、Runner 与执行结果分布。细化首屏比例、品牌色、文字对比度、指标口径、能力卡、
  四阶段执行链路和 Lite/Full 部署说明；独立 CSS Module 避免影响登录后的界面。
- 无样本成功率显示“—”，生成中与不可用快照不显示默认零值；陈旧/同步失败状态明确提示，不误报
  平台健康。支持键盘手动刷新、15 秒请求超时、无重叠轮询和隐藏页面暂停，传输失败保留上次统计。

### Database, deployment and compatibility

- 无数据库迁移、持久配置、依赖、离线资产种类或 Runner Protocol 变更；公开统计 API 及脱敏边界不变，
  登录后的工作概览与 App Shell 不受影响。

### Validation and known limitations

- 公开统计单元测试与未登录首页 9 项 Playwright 测试通过，覆盖真实生产构建、初始化/登录入口、空值与
  零成功率口径、大数值、生成中/失败/陈旧快照、键盘重试、单请求超时恢复和页面可见性轮询。测试已纳入
  统一 E2E 入口；全仓格式、lint 与类型检查通过。人工审查 1024×768 与 1536×960 的
  空数据、大数值、生成中、同步失败和登录入口完整页面截图，未发现错位、截断、控件重叠或横向溢出。
- 另完成 16 组桌面视口/像素密度的 Playwright 审查，覆盖 1024×768 至 3840×2160 及 DPR
  1 / 1.25 / 1.5 / 2，共 48 项正常、空数据和同步失败布局检查通过，并人工查看 28 张截图。
  1024–1920px 常见桌面布局正常；2560px 较为保守，3440px 超宽屏与原生 4K 因内容最大宽度
  1400px 而留白偏多，原生 4K 页尾下方也有明显留白。1180/1181px 断点调整存在间距跳变。
  这些视觉限制不影响入口和统计操作；此次仅验证 Chromium，未覆盖其他浏览器或真实系统缩放。
- 发布前全量 897 项 TypeScript 单元测试、Go 测试和 28 项发布/运维脚本测试通过；刷新按钮复用共享
  UI 组件，保持桌面尺寸、键盘操作和独立首页样式。首页与 JAR 导入联合 Playwright 回归 10 项通过，
  修正初始化后匿名访问测试的会话清除竞态，并重新审查最小桌面与大屏截图。
- 本次仅修改共享 UI 与客户端公开统计同步，无适配器或协议变更；本地未运行 Full 基础设施及 Runner
  端到端全量回归，完整 Full、离线与发布资产验收由版本标签流水线执行。
...
- 稳定用例分析弹窗的布局回归：显式等待入场动画结束，并在同一帧检查正文与操作栏位置，避免未启用截图时产生跨帧坐标误报；保留全部溢出及控件重叠检查。

## 1.15.2 - 2026-09-09

### Fixed

- 修复长用例分析弹窗滚动时底部操作栏遮挡表单的问题，关闭/提交按钮独立于正文滚动区域；补充单条、批量及已完成详情在正文顶部和底部的桌面视口回归。

## 1.15.1 - 2026-09-09

### Fixed

- 修复用例分析弹窗被长用例名称和类路径撑宽、出现横向滚动及操作按钮被挤出的问题，单条、批量和已完成分析统一约束内部网格宽度，完整用例名称自动换行。

## 1.15.0 - 2026-09-09

### Added

- DDT 用例关联 SR 执行类后可立即执行，并通过眼睛图标打开共用详情面板；执行与分析历史按 DDT 用例隔离，Lite/Full 均在线程中创建独立数据快照。

### Fixed

- 修复切换到 DDT 管理后“用例管理”主标题缩小的问题，TestNG 与 DDT 共用页面主标题字号、字重和行高。
- 修复 DDT 用例侧栏宽度固定、工作区高度超出窗口的问题，改为按实际可用空间伸缩，拖动比例与收起状态在窗口变化时保留。
- 修复“开始执行”弹窗滚动时底部操作按钮遮挡输入框的问题，表单内容与操作区独立布局。

### Database, deployment and compatibility

- 相较 v1.14.1 无新增数据库迁移、持久配置、外部服务、Runner Protocol 或离线资产种类；无需因此更新 Runner。
- DDT 新增作用域隔离的摘要、详情、执行与分析历史读取接口，以及单用例执行入口；现有普通用例接口保持兼容。
- DDT 执行读取当前已保存的数据、SR 关联及执行类版本，固化独立 JSON 快照；必须关联可执行测试类并启用 CoTest Adapter，执行要求 `run.create` 权限。

### Validation and known limitations

- 发布前全量 874 项 TypeScript 单元测试、Go 测试、28 项发布/运维脚本测试与 E2E 覆盖清单检查通过；本地全仓集成测试 186 项通过，130 项因未启用对应外部服务而跳过，受影响的 PostgreSQL 专项已另行连接真实数据库验证。
- 应用服务、真实 SQLite/PostgreSQL 仓储与 Web 读取/线程分发共 54 项专项测试通过，覆盖快照、作用域、权限与异常拒绝。
- 10 个不同 Playwright 场景通过，包含真实 Runner/Adapter/TestNG 的 DDT 快捷执行、普通单用例回归与共用执行弹窗；侧栏修复后再次通过全部 7 项 DDT 管理场景。
- 全仓格式、lint、类型检查和 Web 生产构建通过。人工检查 1024×768、1536×1024、2560×1440 的工作区与弹窗截图，以及 1024×960、1536×960 的 DDT 执行和历史截图。
- Full 本地验证范围为真实 PostgreSQL 契约与共享应用/线程分发；完整 Full、离线与发布资产验收由版本标签流水线执行。极矮窗口保留工作区最小操作高度，允许页面滚动。
...
- DDT 用例页采用左右分栏工作台：左侧提供搜索、分组、CaseID 导航和多选，右侧常驻详情、字段卡片、
  旅程 Step 和修改历史；支持收起、拖动或键盘调宽、键盘切换、单字段编辑和复制。
- 多选操作集中到右侧工作区，保留批量修改、导出、回收和加入任务；筛选条件写入 URL，详情按需读取
  并取消过期请求，页面内切换前提示处理未保存的编辑草稿。
- 新增独立 SR 测试类关联页，候选范围按项目、项目版本和测试阶段配置；同 SR 的全部 DDT 用例统一
  继承关联，后续导入、回收恢复与修改 SR 自动适用，已有执行快照保持不变。
- 候选范围与 SR 关联使用修订号及事务防止并发覆盖、范围外关联和移除在用测试类；只读用户可查看，
  修改要求 `case.manage`，保存和冲突通过右上角通知反馈。

### Fixed

- DDT 保存和列表读取不再等待概览统计刷新，避免用例已保存但编辑区仍长时间等待；统计独立更新。
- 修复 Lite 动态字段筛选使用错误 JSON 路径导致查不到用例的问题；中文及含点号、括号、引号、
  反斜杠的列名按字面字段匹配。
- 同一次任务读取跨多个 SQL 窗口时，同 SR 固定首次读取到的测试类关联，避免并发修改导致批次混用
  新旧关联；重复测试类 ID 合并查询。

### Database, deployment and compatibility

- SQLite `0068_ddt_sr_execution.sql` / PostgreSQL `0066_ddt_sr_execution.sql` 新增 SR 关联及候选
  范围表。迁移同时处理活动与回收站用例，一致的旧关联自动继承，不一致的 SR 标记待确认。
- 原逐用例关联字段保留用于升级核对，不再参与新执行或作为回退。逐 CaseID 关联入口移除，旧
  `POST /api/v1/ddt/cases/execution-class` 明确返回 `DDT_SR_MAPPING_REQUIRED`，应改用 SR 关联 API。
- 升级前备份数据库；迁移失败在事务内回滚。降级必须恢复升级前备份，不能只回退程序。
- Lite/Full 共用页面及应用服务；无新增外部服务、环境变量、Runner Protocol 或离线资产种类。

### Validation and known limitations

- 全仓 864 项 TypeScript 单元测试、Go 测试、28 项发布及运维脚本测试和 E2E 覆盖清单校验通过。
- 双数据库仓储、迁移回滚及应用逻辑等 67 项专项测试通过，覆盖范围隔离、并发冲突、历史关联迁移、
  任务读取与导出；全仓格式、lint、类型检查及 Web 生产构建通过。
- 7 项 DDT Playwright 场景通过，覆盖导入冲突、分栏交互、筛选与浏览器返回、候选范围和 SR 继承；
  真实 Runner / Adapter / TestNG 执行通过，验证解绑后已有执行快照不变。人工检查 1024px / 1536px
  工作台、SR 页面及候选范围弹窗截图，修复窄桌面入口重叠后复验通过。
- 升级后标记“旧关联待确认”的 SR，需要管理员在关联页明确选择测试类后才能创建新执行；无历史关联
  的 SR 保持未关联，平台不会自动猜测测试类。

## 1.13.5 - 2026-09-09

### Added and changed

- DDT 重复列处理弹窗新增全部/单组建议改名和“仅保留此列”快捷操作，输入时即时校验空名称、重名及
  `CaseID`/`srNum` 规范列；ZIP 冲突明确展示压缩包与内部表格路径，并在有界滚动区中连续处理多组冲突。
- 用例分析任务在当前用户已有认领时默认进入“我的分析”，没有认领时仍进入认领列表；认领候选与个人分析卡片会为同一任务近 5 次执行中成功过的用例显示醒目标记和最近成功批次提示。
- 项目“成员与角色”列表新增行内“管理角色”弹窗，可直接查看、添加或移除当前项目角色，无需切换到角色权限页面。变更立即撤销目标用户的旧会话；移除最后一个角色会将用户移出项目，当前负责人最后一个项目管理角色继续受服务端保护。
- 用例分析的日志对比取消每 200 行的页面切换，将已读取差异通过虚拟渲染在单个连续页面中展示；保留差异跳转与双侧同步滚动，长日志读取期间显示日志块和字符进度。
- 质量洞察的批次对比明细新增逐用例“日志对比”，复用用例分析的日志流切换、单页差异、差异跳转和同步滚动能力；只有两个批次都产生执行尝试时才可进入。

### Fixed

- DDT 重复列处理入口、校验与失败提示移到相关列表下方；多组冲突卡片保持完整高度并在有界区域滚动，
  底部操作保持可见。“仅保留此列”保留手工改名，重新预检期间冻结表单，失败或暂不处理后保留当前草稿。
- DDT 导入任务显示 CaseID 冲突等任务级失败原因和跳过数量，避免只有“失败”状态而无法判断处理结果。
- 恢复质量洞察批次对比表格的紧凑行距，并调整操作列内边距，避免新增日志对比按钮撑高行高或被截断。
- Next.js 与 ESLint 配置升级到 `16.3.3`，同步更新 sharp 安全补丁和锁定依赖，修复安全扫描报告的
  [Next.js Windows 路径问题](https://github.com/advisories/GHSA-p293-qw3h-jr36)、
  [图像优化问题](https://github.com/advisories/GHSA-2xp9-vwfh-vxw4)及
  [sharp/libheif 问题](https://github.com/advisories/GHSA-rgj7-g3m4-5g8c)。
- 修复 ZIP 中某个表格存在重复列时有效条目被丢弃、损坏条目与冲突条目合并成压缩包级失败，导致无法
  按内部表格解决冲突的问题；现在每个条目独立保留有效、冲突或失败预检结果，解决后使用原 ZIP 重检。
- 修复质量洞察开始批次对比时页面跳回顶部，以及相同批次组合再次点击后一直停留在“正在生成批次对比”的问题；对比更新保留当前位置，相同组合会正常刷新并恢复按钮。

### Database, deployment and compatibility

- 无数据库迁移、持久配置、离线资产种类或 Runner Protocol 变更；主平台镜像包含更新后的锁定依赖，
  无需因此单独升级 Runner。DDT 修复复用 Lite/Full 共享解析器、
  ObjectStore 与导入应用服务，已有未确认预检任务仍可继续处理；成员管理复用现有身份权限应用服务与 API，质量洞察与用例分析共用 Lite/Full 的有界日志读取接口；近 5 次成功提示通过 SQLite/PostgreSQL 共享筛选语义批量读取现有执行事实。

### Validation and known limitations

- DDT 解析器、应用服务、真实 SQLite/PostgreSQL 和隔离线程专项 14 项通过，无跳过。5 项 DDT 浏览器
  闭环通过，覆盖单文件/ZIP、身份列选择、改名后二次冲突、失败重试、草稿保留、CaseID 三种策略、实际
  入库与覆盖历史。人工查看 1024px / 1536px 截图，确认提示在列表下方、滚动和底部操作正常。
- 全仓格式、lint、类型检查、生产构建、861 项 TypeScript 测试、Go 测试和 28 项发布/运维脚本测试通过。
  生产依赖扫描 High/Critical 为零；现存 3 项 Moderate 不属于本次 High/Critical 修复范围。
- 用例分析服务与 SQLite 历史窗口回归共 19 项通过，PostgreSQL 同契约 3 项因本机未配置测试库而跳过；全仓格式、lint、类型检查及 Web 生产构建通过。完整用例分析 Playwright 闭环 1 项通过，并人工审查 1024px/1536px 的认领列表与个人队列截图：成功标记清晰可见，表格行高、卡片对齐、换行和溢出正常。
- 相关 ESLint、Web 与 Playwright 类型检查、全仓格式检查和 Web 生产构建通过；项目角色增减闭环及 1024px/1536px 弹窗布局共 2 项 Playwright 场景通过。
- 日志加载、差异及批次映射相关测试通过；日志对比 3 项 Playwright 场景通过，覆盖加载进度、无分页、跨原分页边界差异跳转、2,000 行虚拟渲染、同步滚动、滚动到底、失败重试、草稿保留、批次对比滚动保持、重复生成和两侧日志复用。人工审查质量洞察 1536px 卡片、1024px 明细，以及 1024px/1536px 日志弹窗截图，未发现溢出、错位或层级问题。
- 弹窗只管理当前项目的项目角色；系统角色及角色定义仍在“角色权限”页面维护。已停用但仍绑定的项目角色只允许移除，不能重新分配。
- 日志对比仍保持单侧最多 400 个日志块、128 Ki 字符和 2,000 行的安全上限；达到上限时连续页面只展示已读取范围并明确提示，不会无界占用浏览器内存。
...
- 修复新批次日志文件已创建但表尚未初始化时立即查看公开/实时日志的数据库异常；等待上传时只读返回空日志，后续查询可正常读取首批内容，实际损坏仍报告错误。
- 修复历史导入 JAR 遮蔽新依赖压缩包的问题。配置压缩包时完全以所选全量包作为业务类路径，不混入历史用例/独立旧依赖，缺失类直接失败；Runner 升级后重建旧版混合类路径缓存。新任务和公开日志单用例重跑使用当前项目版本依赖，既有批次快照保留。升级主平台后通过平台升级 Runner 即可自动更新配套 Adapter。
- 公开日志新增“用例更新时间”，显示所选执行实际使用的依赖压缩包上传/登记时间，支持历史/重跑切换与 UTC 查看。旧快照无记录时明确显示“未记录”，无需数据库迁移。
- 修复 Lite Runner 心跳未使用短锁异步重试的问题，短时数据库竞争释放后可沿用原身份继续心跳，无需重装。持续争锁保留有界失败与后续恢复。
- 空快照队列与无新增分析事实不再获取 SQLite 写锁；Lite / Full 统计事实空闲轮询逐步退避至 30 秒，失败不再跳过独立的首次页面快照构建。
- 区分后台锁退让与实际刷新失败；资源尖峰立即保护但持续压力才通知，同类持续故障按事件合并，连续五分钟恢复后再次发生才重新提示，避免每五分钟重复刷通知。
- Jenkins 进度/结果链接及有效的旧进度链接识别浏览器会话：已登录且拥有对应项目 `run.read` 权限时进入完整控制台执行详情；匿名、会话失效或无权限时保留公开只读页。跨站点击通过有界同源请求补查被 `SameSite=Strict` 隐藏的会话，不降低 Cookie 策略，不改变机器轮询、签名或 Lite/Full 权限边界；无需数据库迁移、配置调整或 Jenkins HPI 更新。

### Database, deployment and compatibility

- 无数据库迁移、新增外部依赖、离线资产类型或 Runner Protocol 变更。持久配置 schema 版本保持 v1；旧配置自动补齐两个默认值，旧版 v1 管理客户端省略新字段时保留服务器当前值。

### Validation and known limitations

- 合并后全仓格式、lint、类型检查及 Web/Worker 生产构建通过；855 项 TypeScript 单元测试、Go 全量测试、28 项发布/运维脚本测试、135 项真实 SQLite/PostgreSQL 与隔离线程集成测试通过。Adapter 的 20 项测试、Java 8 字节码检查和双架构静态 Runner 构建通过。公开日志/Jenkins 执行轮次及平台配置的两项完整 Playwright 场景通过，并检查桌面截图。
- 两个数量配置不会放宽单文件 128 MiB、单请求/ZIP 解压总计 512 MiB、ZIP 10,000 个目录项等独立安全边界。
- 持续锁竞争超出预算仍可返回繁忙；低配宿主机同时承载大批量导入和导出时仍有容量边界。资源告警去重不代表已经排除所有生产故障。
- 全量依赖包必须包含测试类及全部依赖，缺失类不再回退历史 JAR。升级主平台及 Full 独立 Worker 后，通过平台的执行机升级或批量升级同步更新 Runner 配套资源，无需单独操作 Adapter。
...
- 修复 Full 调度与领取之间的 PostgreSQL 死锁：Runner 容量预留使用 `FOR NO KEY UPDATE`，继续串行保护容量，同时允许租约外键的 `KEY SHARE` 锁，消除批次锁与 Runner 外键锁的反向等待。真实数据库回归先持有领取侧外键锁，再验证调度可以提交。
- 修复删除任务全部用例后仍展示旧目录并等待后台快照的问题；空状态直接采用删除成功后的权威用例数，并继续支持立即修改、保存任务配置。Playwright 主动阻塞目录刷新验证该行为。
- 任务目录切换后台快照代次时，将普通用例包和 DDT SR 的原生展开状态与已保存状态同步，避免目录意外折叠、再次点击无法收起。回归主动刷新快照，验证展开保留和收起后卸载用例行。任务生命周期及 DDT 的 5 项 Playwright 回归通过，并人工检查 1024px / 1536px 桌面截图。
- 相同目录快照的状态轮询不再中止尚未完成的分支读取；按快照 ID、代次和分支定位读取生命周期，避免初次加载根节点时取消请求后重复读取，继续保留关闭目录和切换代次时的取消行为。
- 执行轮次浏览器验收在长时间 UI 操作后读取批次详情时，对该幂等 GET 的连接重置最多重试一次；不重试 HTTP 错误，也不放宽 Runner 协议写入断言。
...
- PostgreSQL 的 100,000 用例任务、100,000 执行批次和并发完成聚合三项补充验收通过；执行轮次与身份的六项浏览器回归通过。
...
- SQLite / PostgreSQL 对最近访问的统计快照直接读取，避免缓存命中仍执行 UPSERT 争用写锁；超过访问登记间隔时继续持久化活跃状态并恢复后台刷新。
...
- 双数据库快照契约 18 项、连续十万级容量与读模型测试 4 项通过；100 轮组合热读实测 SQLite 210ms、PostgreSQL 464ms，保留原 3 秒预算。
- 执行轮次、分析闭环、后台刷新中的计划弹框及顶栏布局复测通过；顶栏覆盖 1024 至 1536px 的响应式边界，并人工检查截图。格式、lint、类型检查与生产构建通过。

## 1.11.0 - 2026-09-06

### Added

- 用例分析改为手动创建分析任务：支持从分析工作台、执行历史和终态执行详情开始分析，同次执行共享一张卡片，操作结果通过右上角横幅反馈。
- 分析统计限定到单个执行任务；系统管理员、项目管理员及具备相应权限的角色可将分析任务分配给系统已有用户，保留项目权限、认领和完成状态校验。
- 工作概览、用例目录、任务成员、执行统计、质量洞察、批次对比和用例分析使用数据库后台快照与浏览器会话缓存，显示生成时间并支持手动请求更新。
- 存储清单使用节点本地可重建索引，Full 分页固定到所属平台节点；JAR 预览后台生成，源码展开时按需读取，用户及项目成员使用有界分页和批量关联查询。

### Fixed

- 修复任务详情删除用例后再保存配置时误报他人修改的问题，共享最新修订号且保留真实并发冲突保护。
- 分离执行元数据与全量计数、轮次聚合及分页键计算，避免反复打开列表和详情时同步扫描全部执行记录；冷统计显示准备状态，终态继续等待统计完成。
- 修复 SQLite 后台快照写入与 API Token 等读取后写入事务的锁升级冲突；补齐 PostgreSQL / SQLite 反向引用与分页索引，并修复十万级批次重试计数的重复计算查询计划。
- 分析认领进度读取当前权威计数；取消执行正确触发项目统计失效。存储删除等待分页完整后开放，成功后修补列表与汇总并保留展开状态和滚动位置。

### Database and operations

- SQLite 新增 `0063` 至 `0066`，PostgreSQL 新增 `0062` 至 `0065`：分析任务登记表、可重建快照与分块表、分页索引及用例反向引用索引。
- 已有认领记录自动登记为分析任务；尚未开展分析的历史执行需手动开始分析，不再自动出现在工作台中。
- 升级前停止旧平台 / worker 并备份业务数据库，统一升级 Full 节点后启动。迁移在事务中执行；应用回滚可保留新增表和索引，旧版仍按原有逻辑展示分析工作台。
- 无新增环境变量、持久配置字段或生产依赖。快照和 `.cache/storage-inventory-v1.sqlite` 可重建，不参与执行、权限、删除资格或版本冲突判断。

### Compatibility and offline assets

- Lite 保持无外部服务依赖；Full 快照共用 PostgreSQL 与统一时间基准。Runner Protocol 和任务执行配置契约不变。
- 后端离线镜像纳入读模型工作线程；继续发布原生 amd64 / arm64 镜像、内置双架构静态 Runner、Jenkins 插件、五主机 Compose 部署包、SBOM、清单和签名。

### Tests and known limitations

- 本地格式、lint、类型检查、生产构建及 E2E 覆盖矩阵通过；752 项 Vitest、Go 测试和 28 项 Node 脚本测试通过。SQLite / PostgreSQL 集成测试 253 项通过，7 项未配置 Redis / NATS / MinIO 的专项测试跳过。
- 共享十万级数据测试验证快照生成、重复读取不重新计算、统计准确性、分页、版本失效及权限隔离；浏览器回归与修复后复测覆盖全部 39 个场景，人工查看桌面截图。相关测试已加入 CI 与发布检查。
- 首次快照仍需后台计算，超出浏览器缓存预算的目录可能重新下载；存储索引不代表跨节点统一磁盘清单。一次缩放时的顶栏重叠未在连续坐标检查及最终复测中复现，详见[页面排查报告](./docs/architecture/page-loading-audit.md)。本地未执行完整 Full 中间件及离线验收，由发布流水线继续验证。
