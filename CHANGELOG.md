...
- 无数据库迁移、持久配置、依赖、离线资产或 Runner Protocol 变更；DDT 修复复用 Lite/Full 共享解析器、
...
- DDT 解析器、应用服务、隔离线程与用例管理专项 14 项通过；全仓格式、lint、类型检查及 Playwright
  启动的 Web 生产构建通过。单表格与 ZIP 多条目冲突的 2 项浏览器闭环通过，并人工审查 1024px / 1536px
  截图：压缩包内部路径、多组卡片、滚动边界、输入和底部操作区均无错位、遮挡或横向溢出。
