...
- 优化执行详情的执行机面板：使用完整宽度、轮次概览与 Ant Design 节点卡片，展示尝试次数、结果分布、资源使用率和采集时间；将失败/超时与其他状态明确分开，保留调度日志并增加按权限展示的资源监控入口。用例/执行机切换保持标签位置与已有列表状态。
- 修复操作弹框关闭期间收到后台新快照后，页面错误等待下一次 30 秒轮询才更新的问题；浏览器保留待应用代次，关闭后及时刷新，维持原数据库轮询频率与弹框内编辑状态。

### Database, deployment and compatibility

- 无数据库迁移、持久配置、API、Runner/Adapter 协议或生产依赖变更；Lite/Full 共用页面，更新主平台即可，Full 各平台节点应同步更新，无需升级 Runner。
- 双架构后端离线镜像、部署包、Jenkins 插件、SBOM 与签名清单的资产类型不变。截图、测试数据库与本地构建产物不纳入提交。

### Validation and known limitations

- 新增真实弹框内到达新快照的确定性回归，在旧版稳定复现，修复后连续 3 次通过；完整分析、分配、日志对比与缓存共 10 项 Playwright 场景通过。断言关闭后 5 秒内显示新任务且不增加高频元数据读取，实际审查 1024px/1536px 截图。
- 执行机面板通过 8 节点带数据布局、匿名详情、资源监控和完整轮次执行共 4 项 Playwright 回归；实际查看 1024px/1536px 明暗主题截图，检查长名称、空资源、标签位置和权限入口。相关 31 项单测、Web 生产构建、类型、lint 与格式检查通过；完整源码、Full 分布式与正式离线资产验收以本版本 GitHub Actions 结果为准。
- 初始化继承验收按下拉框的 aria-controls 定位来源列表，避免顶栏同名版本选项造成严格定位冲突；完整继承、失败继续和冲突恢复断言保留。
- Full 管理 CI 单独采用 12 分钟作业预算，覆盖约 6 分钟浏览器验收之外的源码构建、服务启动与清理；修复所有测试已通过却在 8 分钟收尾边界被取消的情况，单项测试超时与断言不变。
- 修正发布后存储目录验收的动画时序：逐层展开前等待父目录有限动画结束，并确认本层已展开，避免连续点击 16 层目录时滚动改变点击位置。保留正常动效、默认收起、深目录布局、刷新与搜索断言；仅修改验收脚本，不替换 v1.18.10 的标签或已发布资产。

## 1.18.11 - 2026-09-26

- 未公开发布。标签源码断网分析验收发现弹框期间快照已就绪但页面延迟刷新的时序问题，停止离线包发布并由 v1.18.12 修复；保留原标签，不改写历史引用。

## 1.18.10 - 2026-09-26

### Added and changed

- 补齐全站 Ant Design 提示、状态标签、空状态、进度、加载、头像、时间线及公开错误页；数字输入、时区候选、用户范围切换和执行机分页使用对应 Ant 组件，保留表单提交与 URL 筛选。
- DDT 导入拖放与侧栏调整分别接入 Upload.Dragger、Splitter；分析图片接入 Image 预览，保留图片粘贴、缩放、键盘关闭与嵌套弹窗。表单约束错误显示在字段旁，继续阻止无效提交。
- 修复倒计时执行弹窗标题被滚走、DDT 冲突提示错位、初始化侧栏浅色文字不可见及字段对齐问题。离线打包扩展至 28 类 Ant Design 组件样式校验，缺少新增组件样式时阻止出包。
- 补充全量浏览器回归，修复 DDT 测试类及分类搜索框过窄、安全审计搜索图标遮挡，以及字段校验容器使目录工具栏和日志搜索异常增高的问题；项目层级下拉展开后定位当前选项，仅滚动菜单，并保留键盘焦点和页面位置。
- 修复通知中心打开后工具提示遮挡关闭按钮，以及日志弹窗日期筛选框越界；已展开按钮的工具提示自动隐藏，日志筛选按字段边界换行，执行详情分页文字保持横排。
- 顶栏新建项目版本后提供初始化向导，测试阶段、运行依赖、普通用例、DDT 用例、SR 关联与用例任务均可跳过；明确提示各步骤可继承内容，并支持从版本下拉菜单重新打开。
- 支持同项目跨版本继承，目标已有用例、分类和关联保持不变；任务成员映射到目标版本，复制的任务默认停用，支持只复制配置。后台有界分批处理，可暂停和失败后继续，重复请求复用已创建内容。
- 顶栏在项目层级切换完成前暂时禁用交互，避免刚打开的下拉菜单被后续刷新收回。

### Database, deployment and compatibility

- Lite/Full 共用初始化流程，分别写入 SQLite/PostgreSQL，无新数据库迁移或持久配置；新增版本初始化 API 和审计记录。无需升级 Runner/Adapter，依赖版本与离线资产类型不变。
- 初始化关闭后保留已写入内容，重新进入时需重新选择来源与内容；不复制执行历史、分析结论、DDT 模板、定时计划或通知绑定。

### Validation and known limitations

- 标准 Web 回归按清单分组运行、定位失败后显式复测，129 个 Playwright 场景最终全部通过，无跳过或失败自动重试；覆盖导入、DDT、任务执行、日志、分析、权限、初始化和管理页面。
- 464 项 Web 测试、401 项应用与契约测试通过；SQLite/PostgreSQL 初始化与 Redis 日志转发专项通过。生产构建、类型、变更源码 lint 与格式检查通过，实际查看 1024px／1536px 等桌面截图。
- 15 项前端离线资源门禁测试通过；生产构建的 83 个静态资源经完整性、SHA-256、28 类组件样式与远程 CSS 引用校验。截图、合成测试数据库和构建产物不纳入提交。
- 本地验证不替代 Full 故障注入、双架构 Docker 或断网安装验收；源码 CI、标签检查与已发布资产验收以本版本对应 GitHub Actions 结果为准。
...
- 修复系统启用“减少动态效果”后通知无法退出并遮挡按钮的问题，保留不可感知的过渡完成事件，确保浮层及时清理。
- 修复减少动态效果下项目下拉菜单定位到视口外的问题，弹层坐标测量不再受到全局过渡覆盖影响。
...
- 修正验收中的模拟 Runner 心跳过期、导入进度截图与取消操作竞态，以及临时 PostgreSQL 数据库强制删除时的连接清理竞态；不通过放宽业务断言或隐藏异常消除失败。
- 完整历史迁移测试统一使用 30 秒有界预算，CI 浏览器及 Release 源码验收使用 15 分钟任务预算，覆盖生产构建和截图证据上传，避免测试通过后仍被旧 8 分钟上限截断。
...
- 大弹窗按实际可用区域约束宽度，修复质量洞察明细在带滚动条的窄桌面视口中被挤到屏幕下方的问题。
...
- CI 发现旧 JAR 与平台设置布局断言将预留滚动条空间误判为横向溢出；统一按浏览器可用内容宽度检查，仍拒绝超出内容边界的页面。
- UI 浏览器验收拆为独立作业，弹窗按扣除滚动条后的布局区域验证居中；后台导出验收校验已持久化的大小、摘要与行数，允许在批次创建前完成的空 CSV，消除对工作器先后顺序的依赖。
- UI 场景独立准备项目层级；CI 中文截图字体改为小体积文泉驿微米黑，避免字体下载占满验收预算，不改变产品依赖或离线资源。
...
- 发布前 CI 补齐多行通知的自适应高度及已读状态排版，避免文字覆盖相邻通知；共享按钮、表单控件、标签页及折叠区在客户端事件处理就绪前保持禁用，避免页面首屏丢失操作，日期选择器首屏使用主题规定的最小控件高度。
...
- Full 的新 JAR 用例改为每批最多 500 条的批量写入，缩短目录导入事务的持锁时间；保留来源导入的原子性、同范围串行合并及历史版本语义，避免并发大批量导入因逐条 SQL 往返耗尽锁重试预算。
...
- 发布前 CI 发现并修复两处测试问题：Runner 安装用例区分 Ant Design 可见选择框与隐藏表单桥接字段；进程树超时测试先等待子进程 PID 就绪，再启动超时，保留存活检查。增加超过原超时窗口的慢启动夹具，修复后连续 30 轮通过；不改变 Agent 生产代码。浏览器普通操作增加 30 秒上限，避免单次被遮挡的点击占满整个流水线时限；长耗时业务保留显式等待预算。
...
- 补充脚本延迟加载的首屏交互回归，以及模拟 SQL 往返延迟的双来源并发导入回归；两项均先复现失败再验证修复。PostgreSQL 与 SQLite 相关仓储测试共 20 项通过，管理页面与长通知另经 4 项 E2E 和 1024px/1536px 实际截图复查。
...
- 补齐 DDT 检索恢复阶段让位高优先级任务后的验收：模拟一次繁忙响应，验证统计进度保留、显式重试及完整分页结果；重试限定为该响应并保留超时预算。
...
- 修复并发生成质量洞察快照时总数、趋势与耗时可能不一致的问题。Lite/Full 在同一个只读快照中汇总各项指标；Full 补齐统计事实不再跳过被其他工作器锁定的已完成执行记录，避免缓存为零的结果。
