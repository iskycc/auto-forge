...
## 17. 日志公开访问页整体深色（v0.6.4）

- 诉求：公开日志页的日志区是深色背景，页面其余部分是白色，黑白对比刺眼。
- 决策：整页改深色，而不是把日志区改白——日志区深色是终端观感的一部分，
  且 ANSI 高亮、日志级别配色都是按深色底设计的。
- 实现：只在 `.share-log-page` 上覆盖语义 token（canvas/surface/text/border、
  状态色与 soft 背景、滚动条、卡片阴影）并设 `color-scheme: dark`；页内复用组件
  （信息卡片、batch-status 徽章、截断警告条、无效链接卡片）全部经由 var() 自动
  跟随深色，无 HTML/API 改动，主站其余页面不受影响（覆盖只作用于该选择器子树）。
- 状态色采用与深色日志区（#17191d）同族的暗色调，soft 背景用 14% 透明度，
  保证徽章与警告条在深底上可读。
