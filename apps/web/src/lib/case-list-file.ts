...
export const MAX_CASE_LIST_FILE_BYTES = 32 * 1024 * 1024;
export const MAX_CASE_LIST_ROWS = 100_001;
...
export async function readCaseListFileColumn(file: CaseListFile): Promise<string[]> {
  ...
  if (lines.length > MAX_CASE_LIST_ROWS + 1)
    throw new Error(`用例列表不能超过 ${MAX_CASE_LIST_ROWS.toLocaleString()} 行。`);
...
async function parseXlsxFirstColumn(bytes: Uint8Array): Promise<string[]> {
  try {
    ...
    if (worksheet.rowCount > MAX_CASE_LIST_ROWS) {
      throw new Error(`首个工作表不能超过 ${MAX_CASE_LIST_ROWS.toLocaleString()} 行。`);
    }
    const cells: string[] = [];
    ...
    throw new Error("无法读取 XLSX，请确认文件未加密、未损坏，并将用例标识放在首列。", {
      cause,
    });
  }
}
