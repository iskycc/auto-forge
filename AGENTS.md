...
- 公开日志、执行分享页与主平台使用一致的 Ant Design 主题，默认浅色，并支持全局深色模式；通过共享 ConfigProvider 和语义 token 切换，深色使用有层次的深灰表面而非纯黑。主题偏好按浏览器保存，服务端首屏必须一致；日志等级、结果状态和长文本仍须保持可读性、分页/截断与滚动边界。
...
- 每次 UI 修改都必须使用 Playwright 在受影响页面生成真实截图，并至少覆盖 1024px 最小桌面视口和一个代表性大屏视口；编码代理必须实际查看截图，检查层级、对齐、间距、换行、溢出、状态与整体观感，不能只以 DOM 断言或测试通过代替视觉审查。交付说明列出截图视口与审查结论。
