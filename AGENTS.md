...
仓库已实现 Lite/Full 用例资产、统一身份/RBAC（含自定义角色与项目成员管理）、可选 LDAP、任务完整执行快照、逐项执行预检和控制面执行协议里程碑：Next.js 主平台、TestNG JAR 静态扫描（含 testng.xml 选择规则、JAR 内继承、Multi-Release 版本选择和工厂/动态语义有界警告）、双数据库用例/身份/执行仓储、本地/MinIO 对象存储、资源感知调度、版本条件保护的权威执行状态机、四阶段超时恢复和批次/attempt 状态历史已经落地；用例元数据编辑与版本历史恢复、任务生命周期（复制/归档/启停/版本快照）与执行策略、来源目录对比/确认同步（保留语义）/归档/守卫式删除与对象异步清理也已接通。Go Runner Agent 已实现注册/资源心跳、版本与工具链兼容提示、assignment claim、lease 续租、启动 reconcile、权威测试/依赖 JAR 下载校验、离线 TestNG 类/方法执行与参数注入、cgroup v2/rlimit 资源限制、日志 spool/确认重传、双层脱敏、安全产物上传、取消/进程组清理、完成上报和直连终端，Runner 凭据轮换/撤销/禁用/排空/注销已在双数据库落地，注销后的执行机记录可通过墓碑式 purge 删除（凭据永久失效、从列表移除、历史执行记录引用保留）；SQLite 持久队列、Lite 嵌入式工作器、PostgreSQL outbox、JetStream 调度和 Redis 可重建缓存也已接通首版，SQLite/JetStream 共享队列契约测试已落地。产品级执行环境和执行密文功能已退役，历史 schema 只为升级/历史读取保留，不得重新暴露页面、API 或新任务输入。严格总磁盘配额、多租户隔离、分析和完整离线验收仍是目标能力。README 中的目标架构不代表这些能力已经完成。
...
- `CaseSuite` 必须保存 Runner 或 Runner Group、项目版本、参数、重跑策略、Adapter 地址和恢复时限；顶栏、计划任务与 Jenkins 只提交 `suiteId` 并直接执行该快照。
...
- 新任务不得注入平台受管密文或任意环境变量；Runner/系统自身凭据不得进入用例进程，日志、错误、事件和产物元数据仍需执行通用秘密脱敏。
...
- 当前产品只支持宽度不小于 1024px 的桌面浏览器；不实现、不验收手机端布局。响应式设计与 E2E 视口矩阵覆盖 1024px 及以上的常见桌面、大屏和超宽屏分辨率。
