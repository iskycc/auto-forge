...
- 用例管理新增独立“DDT 管理”Tab：动态字段、CaseID/srNum、普通 `data` 表格与 `step1…stepN` 用户旅程、XLSX/XLS/XLSB/CSV/ODS/ZIP 导入、中文文件名与编码、逐文件预检、ZIP 条目级重复列内容对照及批量/逐列改名保留或整列删除、覆盖/跳过/报错策略、异步进度/取消/恢复、来源与任务 CaseID 导出、字段模板、永久历史、回收站、批量编辑/导出和项目隔离 API 均已接入 Lite/Full 共享核心。DDT 用例视图采用 CaseID 导航与常驻字段详情的左右分栏，支持旅程 Step、单字段编辑、按需加载和批量操作。独立 SR 测试类关联页按项目版本与测试阶段维护候选测试类范围，支持“SR → 需求分类 → 执行类”映射，可在弹框维护钱包、支付等分类及对应测试类，SR 下所有 DDT 用例共享分类关联，后续导入自动继承；升级时旧关联不一致的 SR 会明确提示人工确认。TestNG 与 DDT 在同一客户端工作台内即时切换，访问过的面板保持挂载，不会清空 TestNG 已加载目录、搜索、勾选或详情状态。重复的用户、LDAP、审计、备份和诊断能力直接复用主平台，不维护第二套事实。详见 [DDT 管理与融合说明](./docs/architecture/ddt-management.md)。
...
- 用例任务采用加大的统计卡片，展示近 7 天执行次数、平均通过率和平均通过用例数；点击“最近执行”在卡片下方按需展开最近 10 次执行，可查看状态、通过/总数、轮次、时间与耗时并跳转详情。次数按批次创建时间统计，均值按已结束批次等权计算，包含异常/终止与最后失败再次执行，排除日志诊断重跑；没有样本时显示“—”。Lite/Full 使用同一统计口径，执行数据受 `run.read` 权限及项目版本隔离保护。
- 用例定义支持展示名、描述、标签与启停编辑，版本历史可查看来源、创建人与变更原因，并允许从旧版本恢复生成新版本而不覆盖历史。用例库支持按 `case.manage` 权限单删和批量删除；删除同步清理任务成员关系，但保留已经物化的执行与分析历史。
- 主导航新增“用例分析”工作台：主页只展示已经手动开始分析的执行任务，支持从“新建分析任务”弹框、执行历史或执行详情创建，同一次执行幂等共享一张卡片并使用右上角横幅反馈；没有分析进展的任务可确认关闭并从列表移除，有进展的任务可确认归档，在同页“已归档”入口按原权限只读查看、统计和导出；关闭保留原执行与日志，归档后的旧页面写入也会被事务校验拒绝。每个任务可进入独立分析详情；详情只列出该任务 `currentRound` 最后一轮仍执行失败的用例，单用例执行、日志诊断重跑、最后失败再次执行、未结束批次和较早轮次失败均不进入分析范围。候选支持按类路径、用例名称、失败堆栈和认领状态进行服务端排序与筛选；同一任务、同一用例在当前批次之前最近 5 次执行中只要有成功结果，认领列表会显示醒目标记。当前用户已经认领该任务下的用例时，再次进入详情默认打开“我的分析”，没有认领时仍进入认领列表；显式 Tab URL 继续优先。“我的分析”可在当前字段/升降序之上按未完成或已完成分组，也可隐藏已完成项，排序、方向、分组与显示开关会保存在当前浏览器并在再次进入时恢复，显式 URL 条件优先。具有 `audit.read` 的系统管理员、项目管理员或自定义高权限角色还可从单次执行的分析任务进入“分析统计”，仅针对该执行分页查看人员认领/完成数量、三类结论占比和逐条填写内容；普通分析人员不能读取跨人员统计。系统管理员、项目管理员及具备 `analysis.assign` 的人员可把未认领用例分配给当前项目有分析权限的已有用户；认领、单个/批量分析、三选一结论、问题说明、修改证明、问题单和备注由 Lite/Full 数据库持久化，并通过 `caseDefinitionId` 回显到用例管理详情。分析弹窗隔离背景交互，单个和批量分析都可复制同时含 HTML 与纯文本的用例信息；一级历史结论仅限同一任务 ID、同一用例 ID，可直接继承；独立的“从本任务近 5 次批跑继承”可搜索当前批次之前最近 5 次已结束标准批跑中所有用例的已完成结论，列表按用例 ID 分页，每个用例只显示最近结论，展开后按需读取近 5 次批跑的其他结论并可逐条继承；搜索旧结论仍显示该用例最新摘要。两种入口均须确认根因一致，提交时在数据库事务内复核各自范围，始终禁止跨任务继承；批量分析也可从本任务近 5 次批跑中选择其他用例结论，经确认后应用到所选用例，证明材料仍逐用例校验且不会被继承。选择“重跑通过”后必须先点击按钮查找公开日志页发起的成功重跑；找到时显示并保存永久日志，未找到的用例必须在分析弹窗任意位置按 `Ctrl+V`（macOS 为 `⌘+V`）粘贴执行通过截图后才能提交。截图通过本地或 MinIO 对象存储保存，不提供点击选择文件入口。主页和详情页均可把最终失败清单导出为紧凑且带统一字体、表头和结论配色的 XLSX，已持久化的责任人、结论、根因、证明和备注会自动回填，未认领/未完成项保留为空。候选和个人队列均使用 50 行游标分页，两个 Tab 按需取数并取消过期请求；列表只传输失败摘要前 8,192 字符，完整日志按需读取，Lite/Full 均有 10 万最终失败记录的性能回归门禁。质量洞察的批次明细可只看结果变化，或指定左右批次的结果组合，并使用紧凑单行列表；逐用例可直接对比两个批次的执行日志，继续复用单页差异与有界读取能力。
- 单个和批量分析弹窗支持查看同一任务、同一用例在本次批次之前的最近 5 个已结束标准批次结果，每批次取该用例当前轮次的最终尝试；按批次创建时间、执行编号倒序，不混入其他任务、日志诊断重跑或最后失败再次执行。历史跨用例版本保留并显示版本号；批量分析按所选用例逐个加载，始终最多 5 条。点击“日志对比”同时打开左右两个日志窗口，展示历史与本次分析尝试的差异、原始行号，支持日志流切换、差异跳转和同步滚动；全部已读取差异在一个连续页面中虚拟渲染，不提供日志分页，长日志读取期间显示加载进度，Esc 返回后保留分析草稿。对比最多读取每侧 400 块、128 Ki 字符并分析前 2,000 行，缺失、截断或达到上限时明确提示；历史读取要求 `run.read`，日志仍由 `log.read` 和项目范围授权。该功能使用现有 Lite/Full 执行与日志仓储，无新增迁移、外部依赖或 Runner 协议变更。
- Runner Agent 注册、身份凭据落盘、周期心跳、在线/离线判定和执行机控制台；支持凭据轮换（旧凭据有明确失效窗口）、撤销、禁用、排空与注销，注销后活跃租约立即到期回收，撤销或注销后心跳、claim、上报与终端均被拒绝。
- 首页以今日工作焦点、关键状态脉冲和六卡 Bento 工作区分层展示真实数据；本周质量包含方法结果构成与耗时，并明确使用当前筛选下最新最多 10,000 条分析事实。用例与质量聚合会按项目版本持久化为可重建首页快照，页面只读取快照；Lite 内嵌 worker 与 Full 独立 worker 在分析事实重建后周期刷新，首次缺失时建立一次，因此重复打开首页不会反复扫描大表。活动执行、执行机组和最近动态仍只读取实际展示所需的有界窗口；质量洞察与导出保持完整精确查询。有 `run.create` 权限的用户可在任意页面通过顶栏“开始执行”发起任务或单用例执行。
- 执行机组可按机房、网络区域或能力维护成员。批次创建时将组成员固化为 Runner ID 快照，后续组变更不会改变历史或运行中的批次；任务和单用例都支持直接选择 Runner 或选择执行机组。
- Runner 页面通过轻量按钮打开自动安装弹窗，不在执行机列表上方平铺 SSH 主机、账号、密码和部署参数。弹窗支持探测 Ubuntu/openSUSE 主机、确认 SHA-256 主机指纹并自动安装平台内置的 `amd64`/`arm64` 静态 Agent 与 CoTest Adapter。远程命令固定使用探测到的 Bash，服务工作目录默认 `/var/lib/autoforge-agent`，安装或重新安装时可显式填写名称、标签、并发、终端策略、运行账号和绝对数据目录；openSUSE 被报告为 SLES 等无法自动确认的场景可由管理员强制选择安装模式。安装资源前后校验摘要，不调用系统包管理器；成功安装后，SSH 主机、端口、用户名、密码和可选私有 CA 以平台主密钥 AES-256-GCM 加密保存，密码/CA 不回传浏览器。已保存档案重新安装时仍必须先探测并确认配置，并通过绑定原 Runner ID 的短期令牌恢复同一逻辑节点，避免任务和执行机组指向废弃身份；最多 50 台的批量升级只替换 Agent/Adapter 程序，远端配置、systemd unit、身份和数据目录保持原字节不变。短期注册令牌成功使用后仍从 Agent 配置原子清除。Agent 重启恢复按协议上限分批核对本地 attempt，直连终端在恢复扫描前建立，assignment claim 在全部核对完成后启动。
- 顶栏提供全局项目切换；独立“配置搜索”按钮按当前权限检索平台、项目、访问和运行配置，点击结果可进入对应模块并聚焦具体平台字段。首页、用例、任务、记录、洞察、来源、审计和项目设置共享同一服务端校验上下文。任务快捷执行只提交 `suiteId` 并使用任务保存的完整配置；单用例快捷执行允许临时选择 Runner/Runner Group、重跑和 Adapter 环境地址，并默认启用 CoTest Adapter。任务与单用例均不提供手工参数覆盖；TestNG 发现参数只读固化。产品级执行环境与执行密文页面、API 和任务字段已退役，新批次不再接受这两类配置。
...
- Jenkins Pipeline 插件提供 `autoforgeRun` 与 `autoforgePublishDependencies` 两个步骤：前者使用 API Key 启动任务，按服务端建议周期检查进度，在可配置总时限内等待批次终态，并返回永久匿名结果链接 `resultUrl`。日志按中文阶段展示轮次、通过/失败、通过率与等待耗时，进度不变时约每分钟提示一次；启动时的“执行详情”与结束时的“完整结果”统一指向同一个永久匿名详情地址，在 Jenkins 经典控制台中作为关键词链接于新标签页打开，不重复铺开长 URL。永久页复用执行历史详情的概览、轮次、图表和用例表格，但不加载产品侧栏或鉴权操作。后者按项目版本替换依赖压缩包链接，不保存历史版本文件，使用中文发布摘要和“下载依赖包”关键词链接，拒绝时显示服务端可操作错误。共享日志库随各自 HPI 内置，无需额外日志插件。两个客户端固定使用 HTTP/1.1，兼容未配置 TLS 代理的 Lite 地址；各插件目录都包含只填写必需参数的 `Jenkinsfile`，ZIP 依赖发布无需重复填写文件名和格式。必填、选填、默认值、权限和网络约束分别见[执行插件参数指南](./integrations/jenkins/autoforge-execution/README.md)与[依赖发布插件参数指南](./integrations/jenkins/autoforge-dependency-publisher/README.md)。两个 HPI、SBOM、校验和与发布清单随 Release 分发，并通过真实 Pipeline DSL E2E 与 HPI 包结构校验。完整流水线见 [`examples/jenkins/Jenkinsfile`](./examples/jenkins/Jenkinsfile)。
- 本地账号首次管理员引导、scrypt 密码、本地/LDAP 统一登录、安全会话、锁定/解锁、密码恢复、六种内置角色和服务端 RBAC；登录来源由服务端根据已有账号来源与 LDAP 启用状态自动判定，同名本地账号优先且目录故障不影响本地紧急管理员。角色、服务账号、项目作用域和 API 令牌权限统一使用带人类友好名称与用途说明的复选框组。自定义角色可创建、编辑、停用与删除（内置角色不可变，引用中角色与最后一位系统管理员受保护，权限变更全量审计并撤销相关会话）；项目支持创建、归档、负责人转移，以及从成员列表弹窗直接增加或减少项目角色。批次、日志、Attempt 时间线、产物下载和取消按权威项目过滤，跨项目 ID 猜测不会读取内容。
...
- SQLite/PostgreSQL assignment、lease、状态事件、完成回执、日志水位和产物元数据，以及原子 claim、续租、回收、reconcile、取消、失败重排和批次聚合；冲突完成会保留事件证据，领取/lease/单用例执行超时使用不同稳定结果码。正常执行完毕统一显示“执行完成”，即使仍有用例失败；基础设施异常和中断分别显示“执行异常”“执行中断”。执行详情可按已分配、执行中、通过、失败、超时、取消和未执行筛选用例，用例搜索框在长关键字和异步同步时保持稳定宽度；单次 attempt 的永久匿名日志页按时间顺序列出同批次、同用例的全部终态轮次，并在当前标签页切换日志。公开日志页固定在浏览器视口内，用例信息与日志正文分别使用独立的自绘滚动条，滚动一侧不会带动另一侧或整个页面。
- 总体与单 Runner 调度日志首次读取最新一页并默认定位末尾，历史事件通过反向游标自动补齐；当前页面内重复打开复用有界 LRU 缓存，新事件按正向游标增量刷新，事件行使用窗口化渲染且不再要求手工“加载更多”。
- Lite/Full 共用安全审计：仅记录用例、任务、项目、用户、授权、平台和执行机等重要数据变更，以及登录、越权拒绝、共享授权和终端访问事件。执行、自动/手工重试、普通查询、预览和连通性测试不进入安全审计；历史运行事件也从审计查询与导出中排除，原有执行状态历史继续保留。新事件持久化中文描述，历史事件按同一目录转换为中文。审计页支持中文搜索、分类/操作/结果与人员/时间筛选、分页、展开详情和中文 CSV 导出，详情仅保留脱敏摘要。
...
- 工作概览、用例与任务成员目录、DDT 概览、质量洞察与批次对比、执行历史与轮次统计、分析任务列表及分析统计使用浏览器缓存和数据库后台快照；请求不再现场计算这些聚合。首次生成自动显示进度，后续访问保留已有内容并后台更新，提供更新时间和“刷新数据”。Lite 独立线程生成 SQLite 快照，Full 通过 PostgreSQL 租约跨节点去重；详见[缓存与后台快照](./docs/architecture/read-model-snapshots.md)。
...
| `DELETE`         | `/api/v1/settings/storage`                                                                                      | 单项或批量删除未被引用的 JDK/依赖资源                                                                  |
...
| `GET`            | `/api/v1/failure-analysis/executions`                                                                           | 按 `projectId`、`analysisId` 查询同任务同用例的前 5 次结果                                             |
| `GET`            | `/api/v1/failure-analysis/conclusions`                                                                          | 按同任务同用例或同任务近 5 次批跑搜索已完成结论                                                        |
| `GET/POST`       | `/api/v1/failure-analysis/claims`                                                                               | 查询个人分析队列或批量认领失败用例                                                                     |
| `GET`            | `/api/v1/failure-analysis/statistics`                                                                           | 管理员分页读取人员分析统计与结论占比                                                                   |
| `GET`            | `/api/v1/failure-analysis/statistics/{claimantId}/claims`                                                       | 管理员分页审阅指定人员填写的分析内容                                                                   |
| `POST`           | `/api/v1/failure-analysis/claims/rerun-proofs`                                                                  | 查找所选用例可作为证明的成功日志重跑记录                                                               |
...
批次、任务和 JAR 来源的完整详情 GET 最多返回 500 个成员、2 MiB JSON；超过上限返回 `413 DETAIL_RESPONSE_TOO_LARGE`。大集合调用方应改为 `view=summary` 与分页子资源，任务仍支持至少十万成员。目录页面按 100 行窗口读取，搜索覆盖完整范围，跨页批量选择单独按需读取标识。缓存语义、兼容性与失败恢复见[后台快照说明](./docs/architecture/read-model-snapshots.md)。
...
Runner/传输基础设施失败使用独立的最多两次自动重调度预算，不占用用例失败重跑额度，并且不等待整轮结束。每次基础设施重调度会产生新的物理 attempt，但仍归入原逻辑轮次；整轮重跑上限为 10 时，页面和批次 `currentRound` 最多只会到第 11 轮。重调度优先避开已对该用例产生异常的 Runner，没有替代节点时才有界回退；每次异常都写入调度事件，执行详情可按执行机、错误和用例聚合查看。“总结”虚拟轮次每个初始用例只保留一行：任一轮通过即计入最终通过，从未通过则保留最后一次失败结果。
...
Runner Agent 由主平台自动生成独立 JSON 配置并以 `--config /etc/autoforge-agent/config.json` 启动，不复用服务端数据库或基础设施凭据。自动安装支持已有 SSH、Bash、systemd 和基础系统命令的 Ubuntu/openSUSE；cgroup v2 可用时自动启用，缺失时使用降级隔离。服务默认使用专用非特权账号，管理员也可显式选择 root 模式；安装过程不会联网或调用系统包管理器。JDK/TestNG 与业务依赖可由项目上传或登记主平台可访问的内网链接，也可使用执行机本地预置工具链作为后备。
...
- 执行耗时和倒计时按平台时间及单调增量计算，不依赖浏览器系统时间。Full 的 Web、调度线程和 worker 共用 PostgreSQL 时间基准，采样中断可保留 120 秒，超期返回 503；Lite 使用进程时间原点，无外部依赖。平台诊断显示当前节点的时间源及宿主机偏差。升级应排空在途任务并同步更新 Runner；历史错误时间不会自动改写，TLS/S3 和中间件仍需可靠校时。详见[统一时间与容错边界](./docs/adr/0013-platform-time-authority.md)。
