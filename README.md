...
- 管理员存储空间页汇总平台实际占用与内容逻辑大小，并按数据目录、对象存储和外部引用直接生成可折叠目录树，不再暴露分页控件；后台使用有界游标批次自动续读，单个目录按需展开，避免大量文件一次性渲染。目录覆盖全部常规文件、Lite/Full 每批次日志 SQLite、Lite 受管对象、Full MinIO 对象及 URL 型 JDK/依赖引用；展开文件可查看逻辑路径、物理位置、大小和磁盘块占用。具有平台设置管理权限的管理员可在二次确认后删除未被项目配置、项目版本或活动批次引用的 JDK/依赖资源；上传资源同时清理对象，URL 资源只清理登记，仍在使用的资源会被拒绝删除。
...
| `DELETE`       | `/api/v1/settings/storage`                                     | 删除未被引用的 JDK/依赖对象或外部资源登记            |
