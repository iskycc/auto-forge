...
- 首页以今日工作焦点、关键状态脉冲和六卡 Bento 工作区分层展示真实数据；本周质量包含方法结果构成与耗时，活动执行聚合多个批次，用例库展示可执行覆盖，执行机组展示实时槽位和资源利用率，失败洞察给出原因占比，最近动态合并执行与导入事件。有 `run.create` 权限的用户可在任意页面通过顶栏“开始执行”发起任务或单用例执行。
