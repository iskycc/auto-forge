...
- 用例管理新增独立“DDT 管理”Tab：动态字段、CaseID/srNum、普通 `data` 表格与 `step1…stepN` 用户旅程、XLSX/XLS/XLSB/CSV/ODS/ZIP 导入、中文文件名与编码、逐文件预检、ZIP 条目级重复列内容对照及批量/逐列改名保留或整列删除、覆盖/跳过/报错策略、异步进度/取消/恢复、来源与任务 CaseID 导出、字段模板、永久历史、回收站、批量编辑/导出和项目隔离 API 均已接入 Lite/Full 共享核心。DDT 用例视图采用 CaseID 导航与常驻字段详情的左右分栏，支持旅程 Step、单字段编辑、按需加载和批量操作。独立 SR 测试类关联页按项目版本与测试阶段维护候选测试类范围，SR 下所有 DDT 用例共享关联，后续导入自动继承；升级时旧关联不一致的 SR 会明确提示人工确认。TestNG 与 DDT 在同一客户端工作台内即时切换，访问过的面板保持挂载，不会清空 TestNG 已加载目录、搜索、勾选或详情状态。重复的用户、LDAP、审计、备份和诊断能力直接复用主平台，不维护第二套事实。详见 [DDT 管理与融合说明](./docs/architecture/ddt-management.md)。
