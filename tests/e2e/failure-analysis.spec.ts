...
test("terminal task failures support durable single and batch analysis with evidence", async ({
  ...
  const startDialog = page.getByRole("dialog", { name: "选择执行任务开始分析" });
  await expect(startDialog.locator("article").filter({ hasText: fixture.suiteName })).toBeVisible();
  await page.setViewportSize({ width: 1024, height: 768 });
  await expectUiIntegrity(page);
  await captureUi(page, "failure-analysis-start-dialog-1024", false);
  await startDialog
    .locator("article")
    .filter({ hasText: fixture.suiteName })
    .getByRole("button", { name: "开始分析" })
    .click();
  await expect(page.getByRole("status")).toContainText("已开始分析");
  ...
  const countTabServerComponentRequest = (request: import("@playwright/test").Request) => {
    if (
      new URL(request.url()).pathname === `/case-analysis/${fixture.batchId}` &&
  ...
  await page.goto("/case-analysis");
  await expect(page.getByText("当前版本尚未创建分析任务")).toBeVisible();
  for (const viewport of [
    { width: 1536, height: 960 },
    { width: 1024, height: 768 },
  ]) {
