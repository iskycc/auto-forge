ALTER TABLE run_batches ADD COLUMN project_id TEXT;
ALTER TABLE runners ADD COLUMN draining INTEGER NOT NULL DEFAULT 0 CHECK (draining IN (0, 1));
...
CREATE INDEX assignments_runner_claim_idx ON assignments (runner_id, status, available_at, priority, created_at);
...
CREATE UNIQUE INDEX attempt_completion_id_uq ON attempt_completion_receipts (completion_id);
...
CREATE UNIQUE INDEX attempt_artifacts_attempt_path_uq ON attempt_artifacts (attempt_id, relative_path);
