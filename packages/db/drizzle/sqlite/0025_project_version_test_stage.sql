...
CREATE UNIQUE INDEX case_sources_legacy_authoritative_uq
  ...
  WHERE authoritative = 1 AND project_version_id IS NULL AND test_stage_id IS NULL;
CREATE UNIQUE INDEX case_sources_stage_authoritative_uq
  ...
  WHERE authoritative = 1 AND project_version_id IS NOT NULL AND test_stage_id IS NOT NULL;
