ALTER TABLE roles ADD COLUMN active INTEGER NOT NULL DEFAULT 1 CHECK (active IN (0, 1));
...
CREATE UNIQUE INDEX case_sources_project_authoritative_uq
  ON case_sources(project_id, authoritative) WHERE authoritative = 1;
...
ALTER TABLE case_definitions ADD COLUMN archived INTEGER NOT NULL DEFAULT 0 CHECK (archived IN (0, 1));
...
ALTER TABLE case_suites ADD COLUMN enabled INTEGER NOT NULL DEFAULT 1 CHECK (enabled IN (0, 1));
