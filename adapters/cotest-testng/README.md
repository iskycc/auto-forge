...
任务配置提供 Suite、Test 和按用例轮询后的环境地址，随后以 `java -jar` 调用；本次批次选定的
完整依赖包是唯一的业务类路径，必须包含测试类、TestNG、CoTest 扩展和全部业务依赖。
Runner 不把历史导入的用例 JAR 或单独声明的旧依赖复制到 `test-jars`，缺失类直接执行失败，
不回退旧包；包内 JAR 按路径确定性排序。未配置压缩包的任务沿用独立测试/依赖 JAR 输入。
每个用例使用独立进程和新建的 ClassLoader，两个用例之间不会共享
已加载的同名业务类。

升级主平台后，通过平台的执行机升级或批量升级更新 Runner；升级流程自动安装配套 Adapter，无需单独升级。
